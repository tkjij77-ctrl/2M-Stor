...
> آخر تحديث: 21 سبتمبر 2026 (T3.1 · T3.2 · T3.3 · T3.7 · F2 · N-2) — هذا الملف يصف **ما ينفّذه الكود الآن**،
...
**ملاحظة:** الجداول الأخرى (`invoice_items` · `profiles` · `audit_log` · `login_attempts`) لا تُبَثّ؛
بنود الفاتورة تأتي مع فاتورتها، والسجل لا يحتاج لحظية، و**`login_attempts` يجب ألّا يُبَثّ أصلًا**:
بثّه سيكشف محاولات الدخول لكل مشترك في القناة — وهو جدول بلا سياسات عن قصد (انظر `docs/rls.md`).
