...
-- الجدول نفسه يبقى مقصورًا على المسجَّلين؛ الزائر يقرأ ما تحتاجه الواجهة فقط.
...
as $$
  select coalesce(jsonb_object_agg(s.key, s.value), '{}'::jsonb)
   ...
   where s.key in (
     'store_name', 'store_phone', 'store_address', 'store_desc',
     'currency', 'return_days', 'return_note',
     'shipping_fee', 'free_shipping_over', 'tax_pct',
     'stock_mode', 'oversell_policy'
   )
$$;

comment on function public.public_settings() is
  'N-2: الإعدادات غير السرّية التي تحتاجها واجهة الزائر (بلا أرقام كوبونات ولا صلاحيات)';
